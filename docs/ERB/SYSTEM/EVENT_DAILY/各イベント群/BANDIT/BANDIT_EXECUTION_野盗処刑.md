# SYSTEM/EVENT_DAILY/各イベント群/BANDIT/BANDIT_EXECUTION_野盗処刑.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/各イベント群/BANDIT/BANDIT_EXECUTION_野盗処刑.ERB`

类型: .ERB

自动摘要: functions: EVENT_DAILY_BANDIT_EXECUTION_RATE, EVENT_DAILY_BANDIT_EXECUTION_DECISION, EVENT_DAILY_BANDIT_EXECUTION_GENRE, EVENT_DAILY_BANDIT_EXECUTION; UI/print
...
ELSE
	...
	ELSE
		...
		IF RESULT == 0
			...
			CALL COLOR_PRINT("その苛烈な処分は多少の賛否を生んだが、おおむね好意的に受け止められた", カラー_注意)
			PRINTFORMW 
		...
		ELSEIF RESULT == 1
			...
			CALL COLOR_PRINT("彼女たちは兵の間で大好評となり、噂を聞いて志願兵が増加した", カラー_注意)
			PRINTFORMW 
		...
		ELSE
			...
			ENDSELECT
			CALL FUCK(対象, "性技, 性交, Ｃ, 射精", "童貞喪失, キス喪失", 0, "野盗の少女の唇", "", @"野盗の少女の膣", "強姦")
			PRINTFORML 
