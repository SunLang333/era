# SYSTEM/EVENT_DAILY/各イベント群/TERRITORY_CEMETERY_墓地の怪.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/各イベント群/TERRITORY_CEMETERY_墓地の怪.ERB`

类型: .ERB

自动摘要: functions: EVENT_DAILY_TERRITORY_CEMETERY_RATE, EVENT_DAILY_TERRITORY_CEMETERY_DECISION, EVENT_DAILY_TERRITORY_CEMETERY_GENRE, EVENT_DAILY_TERRITORY_CEMETERY; UI/print
...
IF ABL:MASTER:武闘 / 10 + RAND:10 + (LOCAL:0 * 5) >= 10
	...
	IF HAS_PENIS(MASTER)
		...
		ELSEIF RESULT == 1
			...
			CALL FUCK(MASTER, "性技, 性交, Ｃ, 射精", "童貞喪失, キス喪失", 0, "死霊術師の唇", "", "死霊術師の膣", "強姦")
			PRINTFORML 
	...
	CITY_GUARD:墓地 += 25
	CALL COLOR_PRINTW("事件を解決して領地の安全を高めた事で墓地の防衛値が上昇した", カラー_注意)
ELSE
	PRINTFORMW 負けてしまった……
	IF IS_MALE(MASTER)
	...
	ELSE
		...
		CALL FUCK(MASTER, "欲望, 奉仕, 性技, 性交, 精愛, Ｃ, Ｖ, Ｂ, Ｍ, Ａ, Ｖ性交, Ａ性交", "キス喪失, 処女喪失, 膣内射精, Ａ処女喪失, 腸内射精, 口内射精", GET_SPERM_ID("野良妖怪"), @"野良妖怪の\@RAND:2 ? ペニス # 唇\@", "野良妖怪", "", "強姦")
		CALL FUCK(MASTER, "欲望, 奉仕, 性技, 性交, 精愛, Ｃ, Ｖ, Ｂ, Ｍ, Ａ, Ｖ性交, Ａ性交", "キス喪失, 処女喪失, 膣内射精, Ａ処女喪失, 腸内射精, 口内射精", GET_SPERM_ID("野良妖怪"), @"野良妖怪の\@RAND:2 ? ペニス # 唇\@", "野良妖怪", "", "強姦")
		PRINTFORML 
	...
	ENDIF
	CALL COLOR_PRINTW("事件は解決できず領民の不安が高まった結果、墓地の防衛値が減少した…", カラー_注意)
ENDIF
DVAR:墓地の怪_発生フラグ = 1
