# SYSTEM/EVENT_DAILY/各イベント群/GAMBLING_SLOT.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/各イベント群/GAMBLING_SLOT.ERB`

类型: .ERB

自动摘要: functions: GAMBLING_SLOT, PRINT_CASINO_SLOT_README; UI/print
...
IF MATCH(回転中, 1) == 0 && MATCH(残回転, 0) == VARSIZE("残回転")
	...
	IF GROUPMATCH("犯", リール:0:(ライン:0), リール:1:(ライン:1), リール:2:(ライン:2))
		PRINTFORMW 犯の字が出てしまった……
		RETURN 2
	ELSEIF ALLSAMES(リール:0:(ライン:0), リール:1:(ライン:1), リール:2:(ライン:2))
		PRINTFORMW 「おめでとうございます！　%リール:0:(ライン:0)%が揃いましたね」
		RETURN 1
	ELSE
		PRINTFORMW 「何もそろいませんでしたねぇ……」
		RETURN 0
	ENDIF
	プレイ中 = 0
	CLEARLINE LINECOUNT - START_LINE
...
CALL SINGLE_DRAWLINE
PRINTFORML 「一般的なスロットです」
PRINTFORML 「何かの絵柄をそろえれば%ANAME(MASTER)%様の勝利、そうでなければこちらの勝利です」
PRINTFORML 「そして『犯』の柄が一つでも出た場合……ペナルティ付きで、こちらの勝利となります」
CALL SINGLE_DRAWLINE
```
