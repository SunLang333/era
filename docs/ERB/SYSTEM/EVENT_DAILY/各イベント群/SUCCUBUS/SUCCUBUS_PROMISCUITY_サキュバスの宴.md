# SYSTEM/EVENT_DAILY/各イベント群/SUCCUBUS/SUCCUBUS_PROMISCUITY_サキュバスの宴.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/各イベント群/SUCCUBUS/SUCCUBUS_PROMISCUITY_サキュバスの宴.ERB`

类型: .ERB

自动摘要: functions: EVENT_DAILY_SUCCUBUS_PROMISCUITY_RATE, EVENT_DAILY_SUCCUBUS_PROMISCUITY_DECISION, EVENT_DAILY_SUCCUBUS_PROMISCUITY_GENRE, EVENT_DAILY_SUCCUBUS_PROMISCUITY; UI/print
...
IF RESULT == 2
	...
	CALL COLOR_PRINTW(@"サキュバス達のところに人が集まり、兵数が{兵数}人増えたようだ", カラー_注意)
ELSEIF RESULT == 1
...
ELSEIF RESULT == 0
	...
	IF (MAXBASE:対象:精神力 / 100) * (RAND:10 + 1) >= (10 + DAY / 8) * (RAND:10 + 1) && !GETBIT(TALENT:対象:淫乱系, 素質_淫乱_淫乱) && !GETBIT(TALENT:対象:淫乱系, 素質_淫乱_精液便女)
		...
		FOR LOCAL, 1, MAX_COUNTRY
			...
			ENDIF
		NEXT
		CALL COLOR_PRINTL(@"%ANAME(対象)%の活躍は他国に知れ渡り評価が上がった", カラー_注意)
		CALL COLOR_PRINTW("しかしサキュバス達からはブーイングが上がった", カラー_注意)
	ELSE
		...
		IF IS_MALE(対象) || (HAS_PENIS(MASTER) && RAND:2) || (HAS_PENIS(MASTER) && TALENT:MASTER:男嫌い)
			...
			CALL FUCK(対象, "性技, 性交, Ｃ, 射精", "童貞喪失, キス喪失", 0, "行きずりの女の唇", "", "行きずりの女の膣", "乱交")
			CALL FUCK(対象, "性技, 性交, Ｃ, 射精", "童貞喪失, キス喪失", 0, "行きずりの女の唇", "", "行きずりの女の膣", "乱交")
			CALL FUCK(対象, "性技, 性交, Ｃ, 射精", "童貞喪失, キス喪失", 0, "行きずりの女の唇", "", "行きずりの女の膣", "乱交")
		ELSE
			...
			PRINTFORML その有様は正に本能のままの獣の交尾であり、サキュバスは嬉しそうにそれを眺めていた
			PRINTFORMW しかし%ANAME(対象)%は彼女の視線も気にせずに、ただひたすら貪る様に肉欲に溺れていった…
			CALL FUCK(対象, "欲望, 奉仕, 性技, 性交, 精愛, Ｃ, Ｖ, Ｂ, Ｍ, Ｖ性交, キス, 口淫, 輪姦", "キス喪失, 処女喪失, 膣内射精, 口内射精, CFLAG減少", GET_SPERM_ID("行きずりの男"), @"行きずりの男の\@RAND:2 ? ペニス # 唇\@", "行きずりの男", "", "乱交")
			CALL FUCK(対象, "欲望, 奉仕, 性技, 性交, 精愛, Ｃ, Ｖ, Ｂ, Ｍ, Ｖ性交, キス, 口淫, 輪姦", "キス喪失, 処女喪失, 膣内射精, 口内射精, CFLAG減少", GET_SPERM_ID("行きずりの男"), @"行きずりの男の\@RAND:2 ? ペニス # 唇\@", "行きずりの男", "", "乱交")
			CALL FUCK(対象, "欲望, 奉仕, 性技, 性交, 精愛, Ｃ, Ｖ, Ｂ, Ｍ, Ｖ性交, キス, 口淫, 輪姦", "キス喪失, 処女喪失, 膣内射精, 口内射精, CFLAG減少", GET_SPERM_ID("行きずりの男"), @"行きずりの男の\@RAND:2 ? ペニス # 唇\@", "行きずりの男", "", "乱交")
		ENDIF
		PRINTFORML 
		...
		COUNTRY_SOLDIER:サキュバス += 兵数
		CALL COLOR_PRINTW(@"サキュバス達のところに人が集まり、兵数が{兵数}人増えた様だ", カラー_注意)
	ENDIF
ENDIF
