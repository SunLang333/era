# SYSTEM/EVENT_DAILY/各イベント群/LEWDNESS_CURSE_淫紋.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/各イベント群/LEWDNESS_CURSE_淫紋.ERB`

类型: .ERB

自动摘要: functions: EVENT_DAILY_LEWDNESS_CURSE_RATE, EVENT_DAILY_LEWDNESS_CURSE_DECISION, EVENT_DAILY_LEWDNESS_CURSE_GENRE, EVENT_DAILY_LEWDNESS_CURSE, SELECT_CHARA_LIST_SHOW_LOGIC_LEWDNESS_CURSE; UI/print
...
ELSE
	...
	PRINTFORML 悪魔は頷くと、契約の代償として女を一人選べ、と言ってきた
	PRINTFORML %ANAME(MASTER)%はその言葉の意味を問うも、悪魔はただ笑うだけ
	PRINTFORML その無言の迫力にゴクリと生唾を飲む
	PRINTFORMW 誰にしよう？
	...
	ELSE
		...
		ELSE
			PRINTFORML ……悪魔の契約だ。今更ながら、大丈夫だろうか……
			CALL ASK_YN("彼女のことを信じる", "それでも力を欲する")
			IF RESULT == 0
				PRINTFORML ……悪魔にどうにかされる彼女ではないと信じよう
				DVAR:淫紋_純愛フラグ = 1
			ELSE
				PRINTFORML いささか不安はあるが、力への欲求は抑えられる物ではない……
			ENDIF
			PRINTFORMW %ANAME(対象)%の名を悪魔に告げた
		...
		CALL COLOR_PRINT(@"%ANAME(MASTER)%の武闘値が{LOCAL}上昇した！", カラー_オレンジ)
		PRINTFORML 
		CALL COLOR_PRINT(@"%ANAME(MASTER)%の防衛値が{LOCAL:3}上昇した！", カラー_オレンジ)
		PRINTFORML 
		CALL COLOR_PRINT(@"%ANAME(MASTER)%の知略値が{LOCAL:1}上昇した！", カラー_オレンジ)
		PRINTFORML 
		CALL COLOR_PRINT(@"%ANAME(MASTER)%の政治値が{LOCAL:2}上昇した！", カラー_オレンジ)
		PRINTFORMW 
		...
		IF 対象 == MASTER
			...
			CALL COLOR_PRINT(@"%ANAME(対象)%に淫紋が刻まれた", カラー_ピンク)
			PRINTFORMW 
		ELSE
			PRINTFORMW %ANAME(MASTER)%は高揚感に包まれながら、再び夢の中へと意識が溶けて行った
			PRINTFORML 
