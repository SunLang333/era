# SYSTEM/EVENT_DAILY/各イベント群/GAMBLING_BINGO.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/各イベント群/GAMBLING_BINGO.ERB`

类型: .ERB

自动摘要: functions: GAMBLING_BINGO, PRINT_CASINO_BINGO_README; UI/print
...
ELSE
	SELECTCASE RESULT
		CASE 997
			PRINTFORMW 「では……」
			PRINTFORMW 「次の玉は……\@プール:玉数 == -1 ? 『犯』 # %TOSTR(プール:玉数)%\@！」
			IF プール:玉数 == -1
				PRINTFORMW 「おや……『犯』ですか……」
				RETURN 2
			ENDIF
			...
			$NANAME_END2
			IF 玉数 == 10
				PRINTFORMW 「全ての玉が出尽くしました……」
				PRINTFORMW 「ゲーム終了です」
				PRINTFORMW 「ビンゴはありませんでしたので、我々の勝利ですね」
				RETURN 0
			ENDIF
...
CALL SINGLE_DRAWLINE
PRINTFORML 「オーソドックスなビンゴゲームです」
PRINTFORML 「10個の玉が出るまでに、縦、横、斜めを揃えれば%ANAME(MASTER)%様の勝利です」
PRINTFORML 「揃わなければ、我々の勝利となります」
PRINTFORML 「ただし、途中で『犯』の玉が出た場合、強制的に我々の勝利です」
PRINTFORML 「またその場合は、ちょっとした『ペナルティ』を受けていただくことになります」
CALL SINGLE_DRAWLINE
```
