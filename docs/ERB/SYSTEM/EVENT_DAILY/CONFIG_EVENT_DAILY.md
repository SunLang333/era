# SYSTEM/EVENT_DAILY/CONFIG_EVENT_DAILY.ERB — 自动生成文档

源文件: `ERB/SYSTEM/EVENT_DAILY/CONFIG_EVENT_DAILY.ERB`

类型: .ERB

自动摘要: functions: INIT_CONFIG_DAILY, CONFIG_EVENT_DAILY; UI/print
...
FOR LOCAL, 0, DAILY_GENRE_NUM
	...
	FOR LOCAL:1, 0, VARSIZE("コンフィグ所属", 1)
		SIF コンフィグ所属:LOCAL:(LOCAL:1) == -1
			BREAK
		SIF GET_DAILY_INDEX_FROM_ENG(デイリー_通常, DAILY_DISABLE_NAME:(コンフィグ所属:LOCAL:(LOCAL:1))) == -1
			CONTINUE
