# SYSTEM/SP_COUNTRY/BANDIT/BANDIT_TRAIN_FUNCTIONS.ERB — 自动生成文档

源文件: `ERB/SYSTEM/SP_COUNTRY/BANDIT/BANDIT_TRAIN_FUNCTIONS.ERB`

类型: .ERB

自动摘要: functions: BANDIT_DOES_TRAIN, BANDIT_FALLEN_MSG, BANDIT_FALLEN_LEAVE_MARK; UI/print

前 200 行源码片段:

```text
﻿;-------------------------------------------------
;調教関数存在判定
;-------------------------------------------------
@BANDIT_DOES_TRAIN()

;-------------------------------------------------
;調教陥落時メッセージ
;-------------------------------------------------
@BANDIT_FALLEN_MSG(対象)
#DIM 対象
IF !GETBIT(TALENT:対象:特殊勢力陥落系, 特殊勢力_野盗)
	PRINTFORMW ……長きにわたる調教生活により、%ANAME(対象)%の心は折れてしまったようだ
	PRINTFORMW %ANAME(対象)%は野盗の奴隷となり、寝返った
	IF TALENT:(対象):プライド高い
		PRINTL
		PRINTFORMW 卑屈な表情を浮かべる%ANAME(対象)%に、もはやかつての面影は残っていない
		PRINTFORMW 野盗たちに強要された屈辱的な行いの数々は、%ANAME(対象)%のプライドを完全に打ち砕いてしまったようだ……
		CALL TALENT_LOSE_PRIDE(対象, 1, 1)
	ENDIF
ELSE
	PRINTFORMW ……野盗の調教を受け、%ANAME(対象)%は彼らの奴隷に過ぎない己の身分を思い出した
	PRINTFORMW %ANAME(対象)%は野盗の奴隷に復職した
ENDIF

;-------------------------------------------------
;調教陥落時にピアシングしたりする場合の処理
;-------------------------------------------------
@BANDIT_FALLEN_LEAVE_MARK(対象)
#DIM 対象
SELECTCASE RAND:10
	CASE IS < 4
		CALL SET_PIERCE_RANDOM(対象, 0)
		SIF RESULT != -1
			CALL COLOR_PRINTW(@"%ANAME(対象)%の%GET_PIERCE_NAME(RESULT)%に、服従の証のピアスが取り付けられた……", カラー_ピンク)
	CASE IS < 7
		CALL SET_TATTOO_RANDOM(対象, STR_FOR_TATTOO(-1), 0)
		SIF RESULT != -1
			CALL COLOR_PRINTW(@"%ANAME(対象)%の%GET_TATTOO_NAME(RESULT)%に、「%TATTOO:対象:RESULT%」とタトゥーが刻まれた……", カラー_ピンク)
	CASEELSE
		IF GET_INMOU(対象) >= 陰毛_標準
		CALL COLOR_PRINTW(@"服従の証として、%ANAME(対象)%の陰毛はハート型に整えられた……", カラー_ピンク)
		TALENT:対象:陰毛目標値 = 陰毛_ハート型
		TALENT:対象:陰毛現在値 = 陰毛_ハート型
	ELSE
		CALL COLOR_PRINTW(@"服従の証として、%ANAME(対象)%の陰毛は永久脱毛された……", カラー_ピンク)
		TALENT:対象:陰毛目標値 = 陰毛_パイパン
		TALENT:対象:陰毛現在値 = 陰毛_パイパン
	ENDIF
ENDSELECT
```
