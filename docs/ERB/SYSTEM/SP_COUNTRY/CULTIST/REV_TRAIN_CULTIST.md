# SYSTEM/SP_COUNTRY/CULTIST/REV_TRAIN_CULTIST.ERB — 自动生成文档

源文件: `ERB/SYSTEM/SP_COUNTRY/CULTIST/REV_TRAIN_CULTIST.ERB`

类型: .ERB

自动摘要: functions: CULTIST_REV_TRAIN_REGULAR_EVENT, CULTIST_REV_TRAIN_MSG; UI/print
...
IF RESULT == 0
	...
	IF RAND:3
		...
		SIF RESULT != -1
			CALL COLOR_PRINTW(@"%ANAME(MASTER)%の%GET_PIERCE_NAME(RESULT)%に、信者であることを示すピアスが取り付けられた……", カラー_ピンク)
	ELSE
		CALL SET_TATTOO_RANKED(MASTER, "神の贄", 0, タトゥー_秘部, タトゥー_尻, タトゥー_頬, タトゥー_額, タトゥー_肩, タトゥー_胸, タトゥー_背, タトゥー_腹, タトゥー_腿, タトゥー_足首)
		SIF RESULT != -1
			CALL COLOR_PRINTW(@"%ANAME(MASTER)%の%GET_TATTOO_NAME(RESULT)%に、信者であることを示すタトゥーが刻まれた……", カラー_ピンク)
	ENDIF
