# SYSTEM/SP_COUNTRY/GAIRAI/GAIRAI_TRAIN_FUNCTIONS.ERB — 自动生成文档

源文件: `ERB/SYSTEM/SP_COUNTRY/GAIRAI/GAIRAI_TRAIN_FUNCTIONS.ERB`

类型: .ERB

自动摘要: functions: GAIRAI_DOES_TRAIN, GAIRAI_FALLEN_MSG, GAIRAI_FALLEN_LEAVE_MARK; UI/print
...
SELECTCASE RAND:10
	CASE IS < 3
		...
		SIF RESULT != -1
			CALL COLOR_PRINTW(@"%ANAME(対象)%の%GET_PIERCE_NAME(RESULT)%に、実験動物であることを示すピアスが取り付けられた……", カラー_ピンク)
	CASE IS < 7
