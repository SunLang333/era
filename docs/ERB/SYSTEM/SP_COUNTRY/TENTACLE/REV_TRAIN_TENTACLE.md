# SYSTEM/SP_COUNTRY/TENTACLE/REV_TRAIN_TENTACLE.ERB — 自动生成文档

源文件: `ERB/SYSTEM/SP_COUNTRY/TENTACLE/REV_TRAIN_TENTACLE.ERB`

类型: .ERB

自动摘要: functions: TENTACLE_REV_TRAIN_REGULAR_EVENT, TENTACLE_REV_TRAIN_MSG; UI/print

前 200 行源码片段:

```text
﻿;-------------------------------------------------
;定期イベント
;-------------------------------------------------
@TENTACLE_REV_TRAIN_REGULAR_EVENT(勢力)
#DIM 勢力

SIF FLAG:逆調教回数 == 0 || FLAG:逆調教回数 % 4 != 0
	RETURN 0

PRINTFORML 調教の時間が近づくと、%ANAME(MASTER)%はいつものように触手どもに拘束され、
PRINTFORMW あとは触手に自我を乗っ取られた連中に汚されるのを待つのみとなった
PRINTFORML やってきた触手兵たちは、%ANAME(MASTER)%に何もしなかった
PRINTFORMW 自ら触手に身体を明け渡し、母体となれば、これ以上の調教はしないつもりのようだが……
PRINTFORMW どうするべきだろうか……
CALL ASK_YN("諦めて受け入れる（苗床化)", "そんなのはごめんだ")
IF RESULT == 0
	PRINTFORML %ANAME(MASTER)%が頷くと、触手は一斉に%ANAME(MASTER)%へと群がった
	FOR LOCAL, 0, 5
		CALL FUCK_SP(MASTER, "欲望, 奉仕, 性交, 性技, 精愛, 排泄, Ｖ拡張, Ａ拡張, Ｃ, Ｖ, Ａ, Ｂ, Ｍ, 口淫, キス, 緊縛, 苦痛快楽, マゾ, Ｖ性交, Ａ性交", "処女喪失, 膣内射精, Ａ処女喪失, キス喪失, 腸内射精, 口内射精", 勢力, GET_SPERM_ID("触手"), "触手", "触手", "", "輪姦")
	NEXT
	SETCOLOR カラー_注意
	PRINTFORMW %ANAME(MASTER)%は触手に寄生されました
	RESETCOLOR
	SETBIT TALENT:MASTER:特殊勢力陥落系, 特殊勢力_触手
	CALL CHANGE_COUNTRY(MASTER, 勢力, 1)
	CALL PLAYER_FALLEN_TO_SP_COUNTRY(勢力)
	RETURN 1
ELSE
	PRINTFORML そんなことが認められるか
	PRINTFORML 拒否の言葉を理解しているのか、触手兵たちは%ANAME(MASTER)%の言葉を受け、再び動き始めた
	PRINTFORMW 彼らはそのまま、%ANAME(MASTER)%に群がっていく……
ENDIF

;-------------------------------------------------
;知り合いがきたときのイベント
;-------------------------------------------------
@TENTACLE_REV_TRAIN_MSG(勢力, 知り合い参加者, 知り合い参加数)
#DIM 勢力
#DIM REF 知り合い参加者
#DIM 知り合い参加数

CALL SINGLE_DRAWLINE
PRINTFORML 調教の時間が近づくと、%ANAME(MASTER)%はいつものように触手どもに拘束され、
PRINTFORMW あとは触手に自我を乗っ取られた連中に汚されるのを待つのみとなった
PRINTFORMW やってきた触手兵の中に、%ANAME(MASTER)%が良く見知った相手が混じっていた
PRINTFORMW %ANAME(知り合い参加者:0, 知り合い参加数)%…
PRINTFORMW 虚ろな表情を浮かべている。その瞳からは理性がうかがえない
PRINTFORMW %ANAME(MASTER)%は必死に呼びかけるが、やはり無反応だった
PRINTFORMW どうやら触手に寄生され、自我を喪失してしまっているようだ……
PRINTFORMW %ANAME(知り合い参加者:0, 知り合い参加数)%は他の触手兵とともに、%ANAME(MASTER)%に群がりはじめた……
```
