# SYSTEM/_FUNCTION_PIERCE.ERB — 自动生成文档

源文件: `ERB/SYSTEM/_FUNCTION_PIERCE.ERB`

类型: .ERB

自动摘要: functions: SET_PIERCE, REMOVE_PIERCE, GET_PIERCE_NAME, SET_PIERCE_RANKED, SET_PIERCE_RANDOM, CAN_PIERCE, CAN_PIERCE_ANYWHERE, IS_PIERCED, IS_HOLED, TOSTR_PIERCE, PRINT_PIERCE, CLEAR_PIERCE_HOLE, CLEAR_PIERCE_HOLE_SEX_CHANGE; UI/print
...
IF !IS_HOLED(ARG:0, ARG:1)
	SIF ARG:2
		CALL COLOR_PRINTW(@"%ANAME(ARG:0)%の%LOCALS:0%に穴が開けられた……", カラー_注意)
	LOCAL:0 ++
	SETBIT TALENT:(ARG:0):ピアス穴, ARG:1
ENDIF
SIF ARG:2
	CALL COLOR_PRINTW(@"%ANAME(ARG:0)%の%LOCALS:0%にピアスが取り付けられた……", カラー_注意)
...
IF IS_PIERCED(ARG:0, ARG:1)
	SIF ARG:2
		CALL COLOR_PRINTW(@"%ANAME(ARG:0)%の%GET_PIERCE_NAME(ARG:1)%からピアスを外した……", カラー_注意)
