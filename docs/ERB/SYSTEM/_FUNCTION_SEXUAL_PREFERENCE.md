# SYSTEM/_FUNCTION_SEXUAL_PREFERENCE.ERB — 自动生成文档

源文件: `ERB/SYSTEM/_FUNCTION_SEXUAL_PREFERENCE.ERB`

类型: .ERB

自动摘要: functions: CHECK_SEXUAL_PREFERENCE; UI/print

前 200 行源码片段:

```text
﻿;--------------------------------
;性的嗜好の変化を行う
;--------------------------------
@CHECK_SEXUAL_PREFERENCE(対象)
#DIM 対象
#DIM 嗜好数
VARSET 嗜好数

FOR LOCAL, 0, VARSIZE("SEXUAL_PREFERENCE_EXP")
	SIF GETBIT(SEXUAL_PREFERENCE:対象:0, LOCAL)
		嗜好数 ++
NEXT

FOR LOCAL, 0, VARSIZE("SEXUAL_PREFERENCE_EXP")
	;嗜好獲得に必要な最低経験値は100。ただし嗜好をふたつ獲得するごとに50ふえていく
	IF !GETBIT(SEXUAL_PREFERENCE:対象:0, LOCAL) && SEXUAL_PREFERENCE_EXP:対象:LOCAL > 100 + 嗜好数 / 2 * 50
		PRINTFORML %ANAME(対象)%の様子がおかしい……
		CALL COLOR_PRINTW(@"様々な性経験を経て、%ANAME(対象)%は", GETDEFCOLOR(), @"<%SEXUAL_PREFERENCE_STR:LOCAL%>", カラー_ピンク, "と感じるようになったようだ……", GETDEFCOLOR())
		SETBIT SEXUAL_PREFERENCE:対象:0, LOCAL
		;嗜好数1->2みたいな変動をしたときのために嗜好数を増やす
		;一度に複数個取得可能な状態になることはまずないと思われるが、仕様としてはこうするのが正しいので
		嗜好数 ++
	ENDIF
NEXT
```
