# SLG/SLG_CITY.ERB — 自动生成文档

源文件: `ERB/SLG/SLG_CITY.ERB`

类型: .ERB

自动摘要: functions: SHOW_CITY_INFO, SPACER; UI/print
...
ELSE
	...
	IF LOCAL:4
		LOCALS:0 = 兵数: %NUM_FORMAT(CITY_SOLDIER:(ARG:0)), 14, LEFT%
	ELSE
		LOCALS:0 = 兵数:????
	ENDIF
	SPACE = 20
	...
	PRINTFORM %LOCALS:0%
	PRINTFORM %TOSTR_SPACE(SPACE - STRLENS(LOCALS:0))%
	PRINTFORM 防衛: %DECIMAL_STRING(CITY_GUARD:(ARG:0), 2)%(都市)
	...
	PRINT ×
	IF FLAG:観戦モード || LOCAL:6 == CFLAG:MASTER:所属
		IF TECHNOLOGY_STATUS:(CFLAG:MASTER:所属):TECHNOLOGY_BATTLE_PASSIVE >= 7
			IF GET_DEVELOPMENT_COUNT(建造物_本拠地, (LOCAL:6)) > 0
				PRINT 1.25
				LOCAL:10 = 125
			ELSE
```
