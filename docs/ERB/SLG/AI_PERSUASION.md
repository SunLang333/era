# SLG/AI_PERSUASION.ERB — 自动生成文档

源文件: `ERB/SLG/AI_PERSUASION.ERB`

类型: .ERB

自动摘要: functions: AI_PERSUASION_CREATE_PLAN; UI/print
...
FOR LOCAL, 0, AI_PERSUASION_MAX_TAREGET
	...
	IF (CFLAG:対象:所属 && REL_LIKE:対象:勢力君主 - REL_HATE:対象:勢力君主 > REL_LIKE:対象:対象君主 - REL_HATE:対象:対象君主 + 300) || (CFLAG:対象:所属 == 0 && REL_LIKE:対象:勢力君主 - REL_HATE:対象:勢力君主 > 300)
		PRINTFORML %ANAME(勢力君主)%は捕らえた%ANAME(対象)%を説得している……
		CALL COLOR_PRINTL(@"説得の末、%ANAME(対象)%は移籍の意志を固めた", CFLAG:対象:所属 > 0 && CFLAG:対象:所属 == CFLAG:MASTER:所属 ? カラー_警告 # カラー_注意)
		CALL COLOR_PRINTL(@"%ANAME(対象)%が%ANAME(勢力君主)%に移籍した", CFLAG:対象:所属 > 0 && CFLAG:対象:所属 == CFLAG:MASTER:所属 ? カラー_警告 # カラー_注意)
