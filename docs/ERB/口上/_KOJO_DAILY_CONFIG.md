# 口上/_KOJO_DAILY_CONFIG.ERB — 自动生成文档

源文件: `ERB/口上/_KOJO_DAILY_CONFIG.ERB`

类型: .ERB

自动摘要: functions: CONFIG_KOJO_DAILY, SET_ALL_KOJO_DAILY, CONFIG_KOJO_DAILY_EACH, SELECT_CHARA_LIST_SHOW_LOGIC_CONFIG_KOJO_DAILY, LISTUP_KOJO_DAILY_EACH, TOGGLE_KOJO_DAILY, SET_ALL_KOJO_DAILY_EACH; UI/print
...
CALL LISTUP_KOJO_DAILY_EACH(対象, 0)
...
CALL TOGGLE_KOJO_DAILY(対象, RESULT)
...
#DIM 対象
RETURN IS_TOHO_CHARA(対象)

@LISTUP_KOJO_DAILY_EACH(対象, ボタン値)
#DIM 対象
#DIM 対象NO
#DIM EVENT_NUM
#DIM ボタン値
...
FOR LOCAL, 0, EVENT_NUM
	...
	PRINTFORML [{LOCAL + ボタン値}] - %EVENT_NAME_JAP, 40, LEFT% - \@ オフ ? オフ # オン\@
NEXT

@TOGGLE_KOJO_DAILY(対象, 指定NO)
#DIM 対象
#DIM 配列要素
#DIM EVENT_NUM
#DIM オフ
...
SIF !INRANGE(指定NO, 0, EVENT_NUM)
	RETURN 0

EVENT_NAME_ENG = %KOJO_DAILY_EVENT_NAME_ENG:対象NO:配列要素:指定NO%
...
@SET_ALL_KOJO_DAILY_EACH(対象, オンオフ)
#DIM 対象
#DIM 配列要素
#DIM EVENT_NUM
#DIMS EVENT_NAME_ENG
...
FOR LOCAL, 0, EVENT_NUM
	EVENT_NAME_ENG = %KOJO_DAILY_EVENT_NAME_ENG:対象NO:配列要素:LOCAL%
	CALL KOJO_DAILY_SET_DISABLE(対象, EVENT_NAME_ENG, オンオフ)
NEXT
