# 口上/70 妖忌口上/DAILY/_KOJO_DAILY_K70_孫娘との再会.ERB — 自动生成文档

源文件: `ERB/口上/70 妖忌口上/DAILY/_KOJO_DAILY_K70_孫娘との再会.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K70_NTR_HIS_GRAND_DAUGHTER_RATE, KOJO_DAILY_K70_NTR_HIS_GRAND_DAUGHTER_DECISION, KOJO_DAILY_K70_NTR_HIS_GRAND_DAUGHTER_GENRE, KOJO_DAILY_K70_NTR_HIS_GRAND_DAUGHTER; UI/print
...
SELECTCASE RESULT
	...
	CASE 0
		...
		PRINTFORML
		FOR LOCAL, 0, 5
			CALL FUCK(対象, "Ｃ, 射精, 性交, 苦痛快楽, Ｖ挿入, キス", "童貞喪失, キス喪失", 0, @"%ANAME(妖夢)%の唇", "", @"%ANAME(妖夢)%の膣", "調教")
			CALL FUCK(妖夢, "Ｃ, Ｂ, Ｖ, 性交, 奉仕, 性技, 精愛, Ｖ性交, キス, 口淫", @"\@ 避妊 == 0 ? 膣内射精, # \@ キス喪失, 処女喪失", 避妊 == 0 ? GET_ID(対象) # 0, @"%ANAME(対象)%のペニス", ANAME(対象), "", "調教")
		NEXT
		PRINTFORML
		PRINTFORML 
