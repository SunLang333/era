# 口上/53 豊姫口上/豊姫/_KOJO_DAILY_K53_美食家（桃）.ERB — 自动生成文档

源文件: `ERB/口上/53 豊姫口上/豊姫/_KOJO_DAILY_K53_美食家（桃）.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K53_GOURMET_RATE, KOJO_DAILY_K53_GOURMET_DECISION, KOJO_DAILY_K53_GOURMET_GENRE, KOJO_DAILY_K53_GOURMET; UI/print
...
PRINTFORMW %ANAME(対象)%と%ANAME(天子)%が何か話しているようだ……
...
PRINTFORMW 見てみれば、%PRONOUN(対象)%の片手には食べかけの桃が握られている……
PRINTFORMW 「なんでも、この天界の桃っていうのには特別な力があるそうね」
PRINTFORMW 「それかどうかしらないけど、なんだか力が湧いてきた気がするわ」
CALL COLOR_PRINTW(@"%ANAME(対象)%の体力が少し増えた", カラー_注意)
MAXBASE:対象:体力 += 100
