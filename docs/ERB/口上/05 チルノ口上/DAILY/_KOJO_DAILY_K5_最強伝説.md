# 口上/05 チルノ口上/DAILY/_KOJO_DAILY_K5_最強伝説.ERB — 自动生成文档

源文件: `ERB/口上/05 チルノ口上/DAILY/_KOJO_DAILY_K5_最強伝説.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K5_SAIKYOU_LEGEND_RATE, KOJO_DAILY_K5_SAIKYOU_LEGEND_DECISION, KOJO_DAILY_K5_SAIKYOU_LEGEND_GENRE, KOJO_DAILY_K5_SAIKYOU_LEGEND; UI/print
...
FOR LOCAL, 0, 4
	;答えを出しておく
	答え = RAND:4
	CALL COLOR_PRINTL(@"%答え文字列:答え%に避けろ！", カラー_警告)
...
ELSE
	...
	CALL COLOR_PRINTW(@"%ANAME(対象)%はその後、こっそり特訓したようです", カラー_注意)
	CALL COLOR_PRINTW(@"%ANAME(対象)%の%ABLNAME:アップ能力%が5上昇しました", カラー_注意)
	ABL:対象:アップ能力 += 5
ENDIF
