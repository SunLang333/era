# 口上/EVENT_KOJO_DAILY.ERB — 自动生成文档

源文件: `ERB/口上/EVENT_KOJO_DAILY.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_DERIVATION; assigns RESULTS; UI/print
...
FOR LOCAL, 0, CHARANUM
	...
	FOR LOCAL:1, 0, EVENT_NUM
		...
		TRYCCALLFORM KOJO_DAILY_K{対象NO}_%NAME_ENG%_RATE(対象)
			...
			IF RESULTS == ""
				CALL KOJO_DAILY_START(NAME_JAP, 対象)
			ELSE
				CALL KOJO_DAILY_START(RESULTS, 対象)
			ENDIF
...
FOR LOCAL, 0, CHARANUM
	...
	FOR LOCAL:1, 0, EVENT_NUM
		...
		TRYCCALLFORM KOJO_DAILY_K{対象NO}_DERIVATION_%NAME_ENG%_DISABLE(対象)
			...
			IF RESULTS == ""
				CALL KOJO_DAILY_START_DERIVATION(NAME_JAP, 対象)
			ELSE
				CALL KOJO_DAILY_START_DERIVATION(RESULTS, 対象)
			ENDIF
