# 口上/99 雷鼓口上/DAILY/_KOJO_DAILY_K99_昂ぶり.ERB — 自动生成文档

源文件: `ERB/口上/99 雷鼓口上/DAILY/_KOJO_DAILY_K99_昂ぶり.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K99_ESTRUS_RATE, KOJO_DAILY_K99_ESTRUS_DECISION, KOJO_DAILY_K99_ESTRUS_GENRE, KOJO_DAILY_K99_ESTRUS; UI/print

前 200 行源码片段:

```text
﻿;---------------------
;基本的な発生確率(1000分率 100で10%)
;これにコンフィグ項目の発生頻度がかけられるので、必ずしもこの通りにはならない
;---------------------
@KOJO_DAILY_K99_ESTRUS_RATE(対象)
#DIM 対象
RETURN 35


;---------------------
;確率以外の発生判定
;〇期以降になると発生するとか、デイリー側で利用している変数が-1だったら起こさない　みたいなはじき方をするときに使う
;---------------------
@KOJO_DAILY_K99_ESTRUS_DECISION(対象)
#DIM 対象

SIF !HAS_PENIS(MASTER)
	RETURN 0

SIF ABL:対象:欲望 < 3
	RETURN 0

SIF KDVAR:対象:雷鼓_昂ぶり == -1
	RETURN 0

RETURN CHECK_KOJO_DAILY_HAPPEN(対象, 1, 0, 1)

;---------------------
;ジャンル
;コンフィグ設定で使用
;---------------------
@KOJO_DAILY_K99_ESTRUS_GENRE(対象)
#DIM 対象
RETURN デイリー_ジャンル_エロ


;---------------------
;本体
;イベントが発生した場合は1、発生しなかった場合は0を返す
;発生しなかったというのは例えば、特定条件を満たすキャラからランダムに一人選ぶデイリーで、そもそもその条件を満たすキャラが一人もいないみたいなとき
;旧仕様で作ったことある人向けにいうと「旧仕様のデイリー本体冒頭で-1を返すような状況」
;---------------------
@KOJO_DAILY_K99_ESTRUS(対象)
#DIM 対象

IF KDVAR:対象:雷鼓_昂ぶり
	PRINTFORMW 「ただいま。今日も楽しかったわ」
	PRINTFORMW ……%ANAME(対象)%がまたライブに行ってきたようだ
	PRINTFORMW ということは……
	PRINTFORMW 「それで。また、鎮めてくれるかしら……？」
	PRINTFORMW やはりだった
ELSE
	PRINTFORMW 「ただいま。はー、楽しかった！」
	PRINTFORML 昼頃から外出していた%ANAME(対象)%が、夜もだいぶ更けてから戻ってきた
	PRINTFORML 頬が紅潮しているし、汗をかいている。酒を飲んできたわけではないようだが……
	PRINTFORMW 「ライブしてきたの、ライブ。会場の熱気ってすごいから、やっぱり大汗かいちゃうのよねー」
	PRINTFORML そう言いながら%ANAME(対象)%は椅子に身体を投げ出し、シャツの胸元を大きく肌蹴て空気を取り込む
	PRINTFORMW 正直、目のやり場に困るのだが……
	PRINTFORMW 「それでまぁ、暑いのは暑いんだけど、なんていうか違う意味でも熱くなってきちゃうのよね、ライブの後は」
	PRINTFORMW 「あれだけ沢山の人が、自分のこと見て、興奮してるって思うと、ねぇ……？」
	PRINTFORML 気づけば%ANAME(対象)%は、%ANAME(MASTER)%のすぐ後ろに移動していた
	PRINTFORMW 耳元でささやかれる言葉は、やけになまめかしい……
	PRINTFORMW 「まあ、水風呂にでも入れば醒める昂ぶりなのかもしれないけど」
	PRINTFORMW 「それじゃ、つまらないと思わない？」
	PRINTFORMW 彼女の香りが鼻孔をくすぐる
	PRINTFORMW 汗をかいたとは言っていたが、臭うとは感じない。むしろその逆だ……
	PRINTFORMW 「誰か鎮めてくれないかな、なんて思ってるところに、%ANAME(MASTER)%がいる。なんてちょうどいいのかしらね」
	PRINTFORMW 「ね、少しだけ、人助けしてくれないかしら？」
	PRINTFORMW 人助けというのは、つまり「そういうこと」だろうが……
ENDIF

PRINTFORMW さて、どうしようか……
PRINTFORML
CALL ASK_YN("協力する", "やめておく")

CFLAG:対象:好感度 += 300
;全員の調教参加フラグをクリア
CVARSET CFLAG, 6, 0
CFLAG:対象:調教参加フラグ = 1
CFLAG:対象:調教中帰らない = 1
CFLAG:対象:強制友好化 = 1
BASE:対象:体力 = MAXBASE:対象:体力
BASE:対象:気力 = MAXBASE:対象:気力

IF RESULT == 0
	PRINTFORMW 「そう言ってくれると思ったわ」
	PRINTFORMW 「さあ、私を掻き鳴らしてみて……？」
	;強制逆調教フラグをオン
	FLAG:ターンエンド調教 = 1
ELSE
	PRINTFORMW 「あら、駄目よ。こんなに楽しめそうなコンディション、みすみす逃せないもの」
	PRINTFORMW 「ちゃんと楽しませてあげるから、じっとしてなさい……♪」
	PRINTFORMW 断ろうとしたが、押し倒されてしまった……
	FLAG:ターンエンド調教 = 2
ENDIF


;それ以降のデイリーイベント発生を潰す
DAILY_CANCEL = 1
;フラグ
KDVAR:対象:雷鼓_昂ぶり = 1

RETURN 1

```
