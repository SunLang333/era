# 口上/134 ドレミー口上/DAILY/_KOJO_DAILY_K134_歩兵の夢.ERB — 自动生成文档

源文件: `ERB/口上/134 ドレミー口上/DAILY/_KOJO_DAILY_K134_歩兵の夢.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K134_BATTLE_IN_DREAM_RATE, KOJO_DAILY_K134_BATTLE_IN_DREAM_DECISION, KOJO_DAILY_K134_BATTLE_IN_DREAM_GENRE, KOJO_DAILY_K134_BATTLE_IN_DREAM; UI/print
...
IF RESULT == 0
	...
	IF RAND:100 <= 判定値
		IF KDVAR:対象:ドレミー_歩兵の夢_成功 ==0
			...
			CALL COLOR_PRINTW(@"%ANAME(対象)%の夢を通じて歩兵の戦いを理解した！", カラー_注意)
		...
		ELSE
			...
			CALL COLOR_PRINTW(@"%ANAME(MASTER)%の武闘が1上昇しました", カラー_注意)
			CFLAG:対象:好感度 += 500
			ABL:MASTER:武闘 += 1
		ENDIF
	ELSE
