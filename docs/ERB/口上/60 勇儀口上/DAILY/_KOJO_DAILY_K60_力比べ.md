# 口上/60 勇儀口上/DAILY/_KOJO_DAILY_K60_力比べ.ERB — 自动生成文档

源文件: `ERB/口上/60 勇儀口上/DAILY/_KOJO_DAILY_K60_力比べ.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K60_COMPETE_RATE, KOJO_DAILY_K60_COMPETE_DECISION, KOJO_DAILY_K60_COMPETE_GENRE, KOJO_DAILY_K60_COMPETE; UI/print
...
ELSE
	PRINTFORMW 「なるほどアンタが！　女にしてはなかなかやりそうじゃないか、会いに来た甲斐があったよ」
ENDIF
...
CALL COLOR_PRINT("アンタの頼みを一つ聞いてやろう", カラー_注意)
PRINTFORMW 」
...
ELSE
	...
	IF RAND:100 <= 判定値
		...
		SELECTCASE RESULT
			...
			CASE 2
				...
				CALL COLOR_PRINTW(@"%ANAME(対象)%を投獄しました", カラー_注意)
				CALL CAPTURE(対象, CFLAG:MASTER:所属)
			CASE 3
				PRINTFORMW 「ほう、私に勝っておいて、まだ力を欲するときた！　はは、アンタみたいな武辺者は大好きだよ」
				PRINTFORMW 「じゃあそうだな、鬼流の喧嘩の仕方を伝授してあげるよ」
				CALL COLOR_PRINTW(@"%ANAME(対象)%に喧嘩の仕方を教わりました", カラー_注意)
				CALL COLOR_PRINTW(@"%ANAME(MASTER)%の武闘が3上昇しました", カラー_注意)
				CALL LEARN_SKILL_GENERAL_BY_NAME(MASTER, "総力戦＋＋")
				SIF RESULT
					CALL COLOR_PRINTW(@"%ANAME(MASTER)%は＜総力戦＋＋＞を得ました", カラー_注意)
				CFLAG:対象:好感度 += 300
				ABL:MASTER:武闘 += 3
			...
			CASE 4
				...
				CALL COLOR_PRINTW("金100000を得ました", カラー_注意)
				MONEY += 100000
				PRINTFORMW 「じゃ、私はこれで。楽しかったよ！」
				PRINTFORMW %ANAME(対象)%は立ち去っていった……
		ENDSELECT
	ELSE
		...
		CALL COLOR_PRINTW("所持金を全て奪われました", カラー_警告)
		PRINTFORMW 「じゃ、私はこれで。楽しかったよ！」
		PRINTFORMW %ANAME(対象)%は立ち去っていった……
		MONEY = 0
	ENDIF
