# 口上/82 芳香口上/DAILY/_KOJO_DAILY_K82_一度死んだなら.ERB — 自动生成文档

源文件: `ERB/口上/82 芳香口上/DAILY/_KOJO_DAILY_K82_一度死んだなら.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K82_REBORN_UNDEAD_RATE, KOJO_DAILY_K82_REBORN_UNDEAD_DECISION, KOJO_DAILY_K82_REBORN_UNDEAD_GENRE, KOJO_DAILY_K82_REBORN_UNDEAD; UI/print
...
IF KDVAR:対象:芳香_一度死んだなら
	IF !(IS_SLAVE(対象) || IS_LOVER(対象) || IS_SLAVED_BY(対象))
		CALL COLOR_PRINTW(@"再び%ANAME(対象)%が蘇り、%ANAME(青娥)%の元に推参しました", カラー_警告)
		CALL CHANGE_COUNTRY(対象, CFLAG:青娥:所属)
	ELSE
		CALL COLOR_PRINTW(@"再び%ANAME(対象)%が蘇り、%ANAME(MASTER)%の元に推参しました", カラー_警告)
...
ELSE
	...
	IF !(IS_SLAVE(対象) || IS_LOVER(対象)) || IS_SLAVED_BY(対象) || MASTER == NAME_TO_CHARA("青娥")
		...
		CALL COLOR_PRINTW(@"%NAME_FORMAL(対象)%が蘇り、%ANAME(青娥)%の元に推参しました", カラー_警告)
		CALL CHANGE_COUNTRY(対象, CFLAG:青娥:所属)
	ELSE
		...
		CALL COLOR_PRINTW(@"%NAME_FORMAL(対象)%が蘇り、%ANAME(MASTER)%の元に推参しました", カラー_警告)
		PRINTFORMW
		CALL CHANGE_COUNTRY(対象, CFLAG:MASTER:所属)
