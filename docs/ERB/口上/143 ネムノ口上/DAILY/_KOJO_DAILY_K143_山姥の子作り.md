# 口上/143 ネムノ口上/DAILY/_KOJO_DAILY_K143_山姥の子作り.ERB — 自动生成文档

源文件: `ERB/口上/143 ネムノ口上/DAILY/_KOJO_DAILY_K143_山姥の子作り.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K143_PREGNATE_ME_RATE, KOJO_DAILY_K143_PREGNATE_ME_DECISION, KOJO_DAILY_K143_PREGNATE_ME_GENRE, KOJO_DAILY_K143_PREGNATE_ME, EVENTTRAIN_KOJO_DAILY_K143_PREGNATE_ME; UI/print
...
IF IS_MALE(MASTER)
	...
	PRINTFORMW 「そりゃまあ、%ANAME(MASTER)%は実力もあるし、種をもらう相手としてはありだべ？」
...
ELSE
	PRINTFORMW 「%ANAME(MASTER)%。うちが背中流そうか？」
	PRINTFORML 風呂に入っていると、突然%ANAME(対象)%が入ってきた
	PRINTFORML そのこと自体にも驚かされたが、なにより%PRONOUN(対象)%が身体を一切隠さず見せつけるように晒していることに驚かされる
	PRINTFORMW 慌てて目をそらすと、%ANAME(対象)%はうれし気な声をあげる
	PRINTFORMW 「ん、%ANAME(MASTER)%はうちを見て興奮するのか？　噂で聞いたとおりだべ」
	PRINTFORML 何を言っているのかと困惑していると、背中に柔らかな感触を覚えた
	PRINTFORML どうやら%ANAME(対象)%が後ろにぴっとりと寄り添っているらしい。ということはこれは、彼女の胸だ
	PRINTFORMW 背中で触れるだに、柔らかさと弾力が伝わってくる……
	PRINTFORMW 「女に生まれたからには、良い男の子種をもらって、産みたいと思うもんだ」
	...
	PRINTFORMW 「だからこそ山姥っていうのは、よさげな男を見かけたら、すぐ家に招いて『歓待』するわけだ」
	PRINTFORML つまり%ANAME(対象)%は、自分と「そういうこと」を望んでいるらしい
	PRINTFORMW しかし、%ANAME(対象)%とはさほど親しくもないし、なによりも自分は女だ。なんだって自分なのだろうか……
	PRINTFORMW 「そりゃまあ、%ANAME(MASTER)%は実力もあるし、種をもらう相手にしとしてはありだべ？」
	PRINTFORMW 「また、親しい親しくないで我々山姥はものを考えない。優秀か、そうでないかだ」
	PRINTFORMW 「性別だってそうだべ。%ANAME(MASTER)%は子種を吐ける。何も問題は無いべ」
	PRINTFORMW 「それに……こんなにしていうことでもなかろ？」
	PRINTFORMW %ANAME(対象)%が前に手を伸ばしてくる。%ANAME(MASTER)%のモノは、すでに硬くなり始めている
	PRINTFORMW 「な？　誰も損はしない話だべ、お前は気持ちいいし、うちも気持ちいいし、強い子ができる」
	PRINTFORMW 「ちゃあんと『そういう薬』も飲んできて、今なら百発百中だべ」
...
IF RESULT == 1
	...
	CALL COLOR_PRINTW(@"%ANAME(対象)%の合意を押し付けられました", カラー_注意)
	KDVAR:対象:ネムノ_子作り = 1
	RETURN 1
ENDIF
...
ELSE
	PRINTFORML そこまでされて、袖にする事なんて出来ない
ENDIF
