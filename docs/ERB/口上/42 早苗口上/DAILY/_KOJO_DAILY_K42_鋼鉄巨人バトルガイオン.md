# 口上/42 早苗口上/DAILY/_KOJO_DAILY_K42_鋼鉄巨人バトルガイオン.ERB — 自动生成文档

源文件: `ERB/口上/42 早苗口上/DAILY/_KOJO_DAILY_K42_鋼鉄巨人バトルガイオン.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K42_OTAKU_RATE, KOJO_DAILY_K42_OTAKU_DECISION, KOJO_DAILY_K42_OTAKU_GENRE, KOJO_DAILY_K42_OTAKU; UI/print
...
SELECTCASE KDVAR:対象:早苗_鋼鉄巨人
	...
	CASE 1
		...
		ELSE
			...
			CALL COLOR_PRINTW(@"夜更かしの反動で、%ANAME(MASTER)%の武闘が3下がりました", カラー_警告)
	...
	CASE IS < 5
		...
		ENDDATA
		PRINTFORML
		CALL COLOR_PRINTW(@"%ANAME(対象)%からの好感度が上がりました", カラー_注意)
		CALL COLOR_PRINTW(@"夜更かしの反動で、%ANAME(MASTER)%と%ANAME(対象)%の武闘が3下がりました", カラー_警告)
		CALL ADD_COOLTIME(MASTER, 2)
		ABL:MASTER:武闘 = MAX(ABL:MASTER:武闘 - 3, 1)
	...
	CASE 5
		...
		PRINTFORMW 「若い二人が夜な夜な部屋で全くもう！　そういうのは\@ TALENT:対象:正妻 || TALENT:対象:妾 ? ほどほどに # 結婚してからに\@しなさいと言ったろう%ANAME(対象)%！」
		...
		PRINTFORMW 「いや、それは悪いことをしたね……私はてっきり、こう、二人が\@ TALENT:対象:正妻 || TALENT:対象:妾 ? ただれた性生活を送っているのかと # いけない関係にあるのかと \@」
		...
		CALL COLOR_PRINTW(@"%ANAME(神奈子)%にしごかれ、%ANAME(MASTER)%と%ANAME(対象)%の武闘が15上がりました", カラー_注意)
		;（差し引きすると全然上がって）ないです
		ABL:MASTER:武闘 += 15
		ABL:対象:武闘 += 15
		KDVAR:対象:早苗_鋼鉄巨人 = -1
