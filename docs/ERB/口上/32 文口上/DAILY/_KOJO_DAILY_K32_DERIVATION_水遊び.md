# 口上/32 文口上/DAILY/_KOJO_DAILY_K32_DERIVATION_水遊び.ERB — 自动生成文档

源文件: `ERB/口上/32 文口上/DAILY/_KOJO_DAILY_K32_DERIVATION_水遊び.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_K32_DERIVATION_MIZUASOBI_DISABLE, KOJO_DAILY_K32_DERIVATION_MIZUASOBI_DECISION, KOJO_DAILY_K32_DERIVATION_MIZUASOBI; UI/print
	...
	CASE 0
		...
		SELECTCASE RESULT
			CASE 0
				...
				CALL COLOR_PRINTW("お粥のおかげで、風邪が完治しました", カラー_注意)
				CALL COLOR_PRINTW("クールタイムが0になりました", カラー_注意)
				CFLAG:対象:好感度 += 300
				COOLTIME:MASTER:0 = 0
			CASE 1
				...
				CALL COLOR_PRINTW(@"%ANAME(対象)%が料理の特訓をしたようです", カラー_注意)
				CALL COLOR_PRINTW(@"%ANAME(対象)%の料理が5成長しました", カラー_注意)
				CFLAG:対象:好感度 -= 300
				ABL:対象:料理 += 5
		ENDSELECT
	CASE 1
