# 口上/_FUNCTION_KOJO_DAILY.ERB — 自动生成文档

源文件: `ERB/口上/_FUNCTION_KOJO_DAILY.ERB`

类型: .ERB

自动摘要: functions: KOJO_DAILY_START, KOJO_DAILY_START_DERIVATION, GET_KOJO_DAILY_INDEX_FROM_ENG, KOJO_DAILY_GET_DISABLE_CONFIG, KOJO_DAILY_SET_DISABLE, CREATE_KOJO_DAILY_MAP, REGISTER_KOJO_DAILY; UI/print
...
CALL COLOR_PRINTL(@"-%ANAME(ARG:1)%口上デイリーイベント『%ARGS:0%』発生-", カラー_注意)
FORCEWAIT
CALL SINGLE_DRAWLINE
PRINTFORML ・
PRINTFORML ・
...
CALL SINGLE_DRAWLINE
CALL COLOR_PRINTL(@"-%ANAME(ARG:1)%派生口上デイリーイベント『%ARGS:0%』発生-", カラー_注意)
FORCEWAIT
CALL SINGLE_DRAWLINE
PRINTFORML ・
PRINTFORML ・
...
IF ジャンル == デイリー_通常
	...
	CATCH
		SETCOLOR カラー_警告
		PRINTFORMW KOJO_DAILY_REGISTERに渡された%名前%,%アクセサ%のRATE関数が見つかりません
...
ELSE
	...
	CATCH
		SETCOLOR カラー_警告
		PRINTFORMW KOJO_DAILY_REGISTERに渡された%名前%,%アクセサ%のDISABLE関数が見つかりません
		PRINTFORMW あなたがプレイヤーなら、パッチに抜けがないか、競合がないかを確認してください
		PRINTFORMW あなたが開発者なら、RATE関数以下の必須関数を設定したかを確認してください
		RETURN
	ENDCATCH
