# TRAIN/訓練関数.ERB — 自动生成文档

源文件: `ERB/TRAIN/訓練関数.ERB`
...
SIF TALENT:(MPLY:0):サボり魔
	TIMES 乱数基準値:0, 0.8
SIF TALENT:(MTAR:0):サボり魔
	TIMES 乱数基準値:1, 0.8
...
IF 育児フラグ == 0
	...
	IF (能力 == GETNUM(ABL, "武闘")) || (能力 == GETNUM(ABL, "防衛")) || (能力 == GETNUM(ABL, "歌唱"))
		SIF HAS_VAGINA(MPLY:0)
			TALENT:(MPLY:0):Ｖ締まり = LIMIT(TALENT:(MPLY:0):Ｖ締まり + RAND:(TFLAG:18 + 2), 締まり_最低値, 締まり_最高値)
		TALENT:(MPLY:0):Ａ締まり = LIMIT(TALENT:(MPLY:0):Ａ締まり + RAND:(TFLAG:18 + 3), 締まり_最低値, 締まり_最高値)
		SIF HAS_VAGINA(MTAR:0)
			TALENT:(MTAR:0):Ｖ締まり = LIMIT(TALENT:(MTAR:0):Ｖ締まり + RAND:(TFLAG:18 + 2), 締まり_最低値, 締まり_最高値)
