# TRAIN/COMF/COMF391_移動.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF391_移動.ERB`

类型: .ERB

自动摘要: functions: COM_NAME391, COM_ABLE391, COM391, COM_TEXT_BEFORE391, COM_TEXT_LAST391, COM_AVAILABLE_WHEN391; assigns RESULTS; UI/print
...
IF IS_INITIATIVE(MASTER)
	PRINTFORMW %ANAME(MPLY:0)%は%ANAME(MTAR:0)%を連れて%GET_PLACENAME(TFLAG:54)%へと向かいました
ELSE
	PRINTFORMW %ANAME(MPLY:0)%は%ANAME(MTAR:0)%に先導されるようにして%GET_PLACENAME(TFLAG:54)%へと向かいました
ENDIF
