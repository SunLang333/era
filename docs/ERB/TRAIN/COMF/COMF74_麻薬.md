# TRAIN/COMF/COMF74_麻薬.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF74_麻薬.ERB`

类型: .ERB

自动摘要: functions: COM_NAME74, COM_ABLE74, COM74, COM_ORDER_PLAYER74, COM_TEXT_BEFORE74, COM_TEXT_AFTER74, COM_AVAILABLE_WHEN74, COM_PREFERENCE_PLAYER_74, COM_PREFERENCE_TARGET_74; assigns RESULTS; UI/print
...
SELECTCASE GET_COM_INITIATIVE()
	;プレイヤーに主導権
	CASE 0
		PRINTFORML %ANAME(MPLY:0)%は%ANAME(MTAR:0)%%LOCALS:0%に麻薬を打っ%TARGET_STR("た", "てきた")%…
	;ターゲットに主導権
	CASE 1
		PRINTFORML %ANAME(MTAR:0)%は%ANAME(MPLY:0)%%LOCALS:0%に麻薬を打つように命令した
		PRINTFORML %ANAME(MPLY:0)%は言われるまま%ANAME(MTAR:0)%%LOCALS:0%に麻薬を打った…
	;第三者に主導権
	CASEELSE
		PRINTFORML %ANAME(TFLAG:45)%は%ANAME(MPLY:0)%に対して、%ANAME(MTAR:0)%%LOCALS:0%に麻薬を打つように命令した
		PRINTFORML %ANAME(MPLY:0)%は言われるまま%ANAME(MTAR:0)%%LOCALS:0%に麻薬を打った…
ENDSELECT
...
IF GETBIT(TALENT:(MTAR:0):淫乱系, 素質_淫乱_薬物中毒)
	PRINTFORML %ANAME(MTAR:0)%%LOCALS:0%は、待ち望んでいたものを打ち込まれ、心底幸せそうにえへえへと笑っている…
ELSE
	PRINTFORML 薬が回ってきたのか、%ANAME(MTAR:0)%%LOCALS:0%は虚ろな瞳でえへらえへらと笑っている…
ENDIF
