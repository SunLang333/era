# TRAIN/COMF/COMF358_背中を流す.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF358_背中を流す.ERB`

类型: .ERB

自动摘要: functions: COM_NAME358, COM_ABLE358, COM358, COM_ORDER_PLAYER358, COM_TEXT_BEFORE358, COM_TEXT_LAST358, COM_AVAILABLE_WHEN358; assigns RESULTS; UI/print
...
IF MPLY:0 == MASTER
	...
	IF IS_INITIATIVE(MASTER)
		PRINTFORMW %ANAME(MPLY:0)%は手拭いを持って%ANAME(MTAR:0)%の背中を洗った
	;主導権が相手側
	ELSE
		PRINTFORMW %ANAME(MTAR:0)%は%ANAME(MPLY:0)%に背中を洗わせた
	ENDIF

	IF TFLAG:18 == 1
		PRINTFORML %ANAME(MTAR:0)%は気持ち良さそうだ…
	ELSEIF TFLAG:18 == -1
		PRINTFORML が、どうやら強く擦りすぎてしまったらしく、%ANAME(MTAR:0)%は痛みに抗議の声を上げた…
	ENDIF
...
ELSE
	...
	;主導権が相手側
	ELSE
		PRINTFORMW %ANAME(MPLY:0)%は強引に%ANAME(MTAR:0)%に背を向けさせると、手拭いで背中を洗ってきた
	ENDIF

	IF TFLAG:18 == 1
		PRINTFORML %ANAME(MPLY:0)%は楽しそうにしている…
	ELSEIF TFLAG:18 == -1
		PRINTFORML が、気合が入りすぎたのか、強く擦られすぎて%ANAME(MTAR:0)%の背中はヒリヒリと痛んだ…
	ENDIF
ENDIF
