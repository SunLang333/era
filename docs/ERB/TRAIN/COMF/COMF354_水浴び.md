# TRAIN/COMF/COMF354_水浴び.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF354_水浴び.ERB`

类型: .ERB

自动摘要: functions: COM_NAME354, COM_ABLE354, COM354, COM_ORDER_PLAYER354, COM_TEXT_BEFORE354, COM_TEXT_LAST354, COM_AVAILABLE_WHEN354; assigns RESULTS; UI/print
...
IF IS_INITIATIVE(MASTER)
	PRINTFORMW %ANAME(MPLY:0)%は%ANAME(MTAR:0)%と一緒に川で水浴びをした
;主導権が相手側
ELSE
	PRINTFORMW %ANAME(MPLY:0)%は%ANAME(MTAR:0)%に誘われ、一緒に川で水浴びをした
ENDIF

IF TFLAG:18 == 1
	PRINTFORML %ANAME(MTAR:0)%は心から楽しそうに笑っている…
ELSEIF TFLAG:18 == -1
	PRINTFORML %ANAME(MTAR:0)%はあまり楽しくなさそうだ…
ENDIF

PRINTFORM %ANAME(MTAR:0)%の服は濡れて肌に張り付き、その
