# TRAIN/COMF/COMF91_ピアシング.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF91_ピアシング.ERB`

类型: .ERB

自动摘要: functions: COM_NAME91, COM_ABLE91, COM91, COM_ORDER_PLAYER91, COM_ORDER_TARGET91, COM_TEXT_BEFORE91, COM_AVAILABLE_WHEN91, COM_PREFERENCE_PLAYER_91, COM_PREFERENCE_TARGET_91; assigns RESULTS; UI/print
...
SIF ITEM:ピアス <= 0 && TALENT:(MTAR:0):ピアス == 0
	RETURN 0
;プレイヤーが行動不能なら不可
...
PRINTFORML %ANAME(MPLY:0)%は%ANAME(MTAR:0)%におしゃれさせてやることにした……
PRINTFORML
FOR LOCAL, 0, ピアス_最大ビット + 1
	...
	ELSE
		SETCOLOR カラー_選択不可
		PRINTPLAINFORM [{LOCAL:0}] %GET_PIERCE_NAME(LOCAL:0)%につけるためのピアスがない
		PRINTL
		RESETCOLOR
	ENDIF
NEXT
...
ELSEIF CAN_PIERCE(MTAR:0, RESULT) && ITEM:ピアス > 0
	ITEM:ピアス --
...
CALL COM_ORDER_ELEMENT(ARG:0, @"マゾLv{ABL:(ARG:0):マゾ}", ABL:(ARG:0):マゾ * 4)
