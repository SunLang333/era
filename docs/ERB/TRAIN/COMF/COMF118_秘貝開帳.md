# TRAIN/COMF/COMF118_秘貝開帳.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF118_秘貝開帳.ERB`

类型: .ERB

自动摘要: functions: COM_NAME118, COM_ABLE118, COM118, COM_IS_EQUIP118, COM_ORDER_PLAYER118, COM_TEXT_BEFORE118, COM_TEXT_LAST118, COM_AVAILABLE_WHEN118, COM_PREFERENCE_PLAYER_118, COM_PREFERENCE_TARGET_118; assigns RESULTS; UI/print
...
SELECTCASE GET_COM_INITIATIVE()
	...
	CASE 1
		IF MTAR_NUM == 1 && ABL:(MTAR:0):サド >= 3
			PRINTFORMW %LOCALS:1%は嗜虐的な笑みを浮かべ%LOCALS:0%に命令し、無理やり%STR_BODY("陰唇：処女確認" , MPLY:0)%を指先で開かせ%PLAYER_STR("た", "てきた")%…
		ELSEIF MTAR_NUM == 1 && ABL:(MTAR:0):主導度Ｕ < 100
			PRINTFORMW %LOCALS:1%は少し緊張した様子で%LOCALS:0%に命令し、%STR_BODY("陰唇：処女確認" , MPLY:0)%を指先で開かせた…
		ELSE
			PRINTFORMW %LOCALS:1%は%LOCALS:0%に命令し、%STR_BODY("陰唇：処女確認" , MPLY:0)%を指先で開かせた…
		ENDIF
	;第三者に主導権
	CASEELSE
		PRINTFORML %ANAME(TFLAG:45)%は%LOCALS:0%に、%LOCALS:1%に%STR_BODY("陰唇：処女確認" , MPLY:0)%を指先で開くように命令した
ENDSELECT
...
PRINTFORMW %STR_BODY("陰唇：処女確認" , MPLY:0)%を指先で割り開いてみせた……
...
FOR LOCAL:0, 0, MPLY_NUM
	...
	SIF CUM_GET_AMOUNT:(LOCAL:1):射精部位_膣内 > 0
		PRINTFORML 中出しを受けた%ANAME(LOCAL:1)%の%STR_BODY("陰唇", LOCAL:1)%から、白いものがとろりと溢れた……
NEXT
