# TRAIN/COMF/COMF299_忠誠を誓わせる.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF299_忠誠を誓わせる.ERB`

类型: .ERB

自动摘要: functions: COM_NAME299, COM_ABLE299, COM299, COM_AVAILABLE_WHEN299; assigns RESULTS; UI/print
...
IF TCVAR:(MTAR:0):24 >= TCVAR:(MTAR:0):25
	...
	IF !TALENT:(MTAR:0):特殊勢力素質 && IS_SP_COUNTRY(CFLAG:MASTER:所属) && !TALENT:(MTAR:0):特殊勢力陥落系 && !GETBIT(TALENT:(MTAR:0):特殊勢力陥落系, SP_COUNTRY_TO_CONST(CFLAG:MASTER:所属))
		CALL COLOR_PRINTW(@"%ANAME(MTAR:0)%は<%SP_FALLEN_TALENT_NAME:(SP_COUNTRY_TO_CONST(CFLAG:MASTER:所属))%>になった", カラー_注意)
