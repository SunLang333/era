# TRAIN/COMF/COMF73_絶倫丸.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COMF73_絶倫丸.ERB`

类型: .ERB

自动摘要: functions: COM_NAME73, COM_ABLE73, COM73, COM_ORDER_PLAYER73, COM_TEXT_BEFORE73, COM_AVAILABLE_WHEN73, COM_PREFERENCE_PLAYER_73, COM_PREFERENCE_TARGET_73; assigns RESULTS; UI/print
...
SIF MTAR_NUM <= 0
	RETURN 0
;絶倫丸が必要
SIF ITEM:絶倫丸 < MTAR_NUM && ITEM:A_絶倫丸 <= MTAR_NUM
	RETURN 0
...
IF ITEM:A_絶倫丸 <= 0
	ITEM:絶倫丸 = MAX(ITEM:絶倫丸 - MTAR_NUM, 0)
ENDIF
...
SELECTCASE GET_COM_INITIATIVE()
	;プレイヤーに主導権
	CASE 0
		PRINTFORML %ANAME(MPLY:0)%は%ANAME(MTAR:0)%%LOCALS:0%に絶倫丸を飲ませ%TARGET_STR("た", "てきた")%…
	;ターゲットに主導権
	CASE 1
		PRINTFORML %ANAME(MTAR:0)%は%ANAME(MPLY:0)%に絶倫丸を飲ませるように命令した
		PRINTFORML %ANAME(MPLY:0)%は言われるまま%ANAME(MTAR:0)%%LOCALS:0%に絶倫丸を飲み込ませた…
	;第三者に主導権
	CASEELSE
		PRINTFORML %ANAME(TFLAG:45)%は%ANAME(MPLY:0)%に対して、%ANAME(MTAR:0)%%LOCALS:0%に絶倫丸を飲ませるように命令した
		PRINTFORML %ANAME(MPLY:0)%は言われるまま%ANAME(MTAR:0)%%LOCALS:0%に絶倫丸を飲み込ませた…
ENDSELECT
