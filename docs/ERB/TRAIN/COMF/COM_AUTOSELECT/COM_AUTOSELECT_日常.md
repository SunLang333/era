# TRAIN/COMF/COM_AUTOSELECT/COM_AUTOSELECT_日常.ERB — 自动生成文档

源文件: `ERB/TRAIN/COMF/COM_AUTOSELECT/COM_AUTOSELECT_日常.ERB`

类型: .ERB

自动摘要: functions: COM_AUTOSELECT_N; UI/print; definition/data
...
IF CHECK_RANDOMIZED < 1
	IF COM_AUTO_COM_PRIORITY:GET_TRAINNUM("なにもしない") < 10
		COM_AUTO_COM_PRIORITY:GET_TRAINNUM("なにもしない") = 10
	ENDIF
	CHECK_RANDOMIZED += COM_AUTO_COM_PRIORITY:GET_TRAINNUM("なにもしない")
ENDIF

;結果取得
TEMP_COM = COM_AUTO_RANDOMIZED_WEIGHTED_RESULT()
IF TEMP_COM >= ADD_NUM_対象が実行 + 1000
