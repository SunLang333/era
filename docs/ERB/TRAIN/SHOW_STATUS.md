# TRAIN/SHOW_STATUS.ERB — 自动生成文档

源文件: `ERB/TRAIN/SHOW_STATUS.ERB`

类型: .ERB

自动摘要: functions: SHOW_STATUS; UI/print
...
IF FLAG:継続コマンド表示絞り込み
	PRINTBUTTON "[絞り込み:有効]", 5904
ELSE
	PRINTBUTTON "[絞り込み:無効]", 5904
ENDIF
PRINT   
IF FLAG:調教中表示簡略化
...
IF GROUPMATCH(FLAG:調教モード, 調教_会う, 調教_閨, 調教_捕虜調教, 調教_捕虜会話)
	PRINTBUTTON " [実行／対象の反転]", 5900
	PRINTBUTTON " [命令させろ]", 5901
ELSE
	SETCOLOR カラー_選択不可
	PRINTPLAIN  [実行／対象の反転]
	PRINTPLAIN  [命令させろ]
...
PRINTBUTTON "[コマンドフィルタ]", 5905
PRINT   

;★added a help button
PRINTBUTTON "[ヘルプ]", 5997
PRINT   

IF GROUPMATCH(FLAG:調教モード, 調教_会う, 調教_閨, 調教_捕虜調教, 調教_捕虜会話)
	PRINTBUTTON "[ショップ]", 5998
ELSE
	SETCOLOR カラー_選択不可
	PRINTPLAIN [ショップ]
	RESETCOLOR
...
;逆調教
IF GROUPMATCH(FLAG:調教モード, 4, 5, 7)
	PRINTFORML %TOSTR_SPACE(13)%%TOSTR_SPACE(48)%機嫌 体力 気力    敏/鈍   状態
;参加人数が2人以上
ELSEIF TFLAG:44 >= 2
	PRINTFORML %TOSTR_SPACE(13)%<主導権><実行>  <対象>%TOSTR_SPACE(17)%性感/処女%TOSTR_SPACE(20)%機嫌 体力 気力    敏/鈍   状態
;参加人数が1人
ELSE
	;会いに行く
	IF FLAG:調教モード == 0
		IF FLAG:ウフフフラグ
			PRINTFORML %TOSTR_SPACE(13)%<主導権>%TOSTR_SPACE(50)%機嫌 酔い 体力 気力    敏/鈍   状態
		ELSE
			PRINTFORML %TOSTR_SPACE(13)%<主導権>%TOSTR_SPACE(50)%機嫌 酔い
		ENDIF
	;子育て
	ELSEIF FLAG:調教モード == 3
		PRINTFORML %TOSTR_SPACE(13)%<主導権>%TOSTR_SPACE(50)%機嫌
	ELSE
		PRINTFORML %TOSTR_SPACE(13)%<主導権>%TOSTR_SPACE(50)%機嫌 体力 気力    敏/鈍   状態
	ENDIF
ENDIF
