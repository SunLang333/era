# SHOP/SHOP_LIFE/SHOP_LIFE62_諸国訪問.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_LIFE/SHOP_LIFE62_諸国訪問.ERB`

类型: .ERB

自动摘要: functions: SHOP_LIFE_NAME62, SHOP_LIFE_CHECK62, SHOP_LIFE_CHECKCHARA62, SHOP_LIFE_EVENTBUY62, SHOP_LIFE_EVENTBUY_SHOW62, SHOP_LIFE_USERSHOP62, VISIT_EVENT_0_RATE, VISIT_EVENT_0_DECISION, VISIT_EVENT_0, VISIT_EVENT_1_RATE, VISIT_EVENT_1_DECISION, VISIT_EVENT_1, VISIT_EVENT_2_RATE, VISIT_EVENT_2_DECISION, VISIT_EVENT_2; assigns RESULTS; UI/print
...
PRINTFORMW %ANAME(訪問先君主)%の勢力を訪ねた……
PRINTFORMW ……%ANAME(対象)%に出会った
PRINTFORMW 少しの間、世間話をした……

IF !CFLAG:対象:面識
	CALL COLOR_PRINTL(@"%ANAME(対象)%の面識を得ました", カラー_注意)
	CFLAG:対象:面識 = 1
ENDIF
...
CALL COLOR_PRINTL(@"%ANAME(対象)%の好感度が{好感度効果量}上がりました", カラー_注意)
...
IF RESULT == 0
	...
	CALL COLOR_PRINTL(@"%ANAME(対象)%が仲間になりました", カラー_注意)
	CALL COLOR_PRINTL(@"%ANAME(訪問先君主)%との関係が悪化しました", カラー_警告)
...
IF RESULT == 0
	...
	CALL COLOR_PRINTW(@"%ANAME(訪問先君主)%の下に移籍しました", カラー_注意)
	CALL COLOR_PRINTW(@"支度金{支度金}を受け取りました", カラー_注意)
	CALL COLOR_PRINTW(@"%ANAME(GET_COUNTRY_BOSS(CFLAG:MASTER:所属))%との関係が悪化しました", カラー_警告)
