# SHOP/SHOP_LIFE/SHOP_LIFE44_触手部屋.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_LIFE/SHOP_LIFE44_触手部屋.ERB`

类型: .ERB

自动摘要: functions: SHOP_LIFE_NAME44, SHOP_LIFE_CHECK44, SHOP_LIFE_CHECKCHARA44, SHOP_LIFE_EVENTBUY44, SHOP_LIFE_BUTTON_ADD44, SHOP_LIFE_LIST1_BUTTON44, SHOP_LIFE_EVENTBUY_SHOW44, SHOP_LIFE_USERSHOP44, IS_SYOKUSYU_MANAGE; assigns RESULTS; UI/print
...
CALL COLOR_PRINT(" 触手部屋の管理を行うには <妖術知識>・性知識Lv4以上・主人公以外なら触手Lv3以上 が必要です", カラー_注釈)
CALL COLUMN_RIGHT_PRINTL
CALL COLOR_PRINT(" 触手部屋の管理を行うと、毎ターン若干の妖術経験値と触手経験値を得られます", カラー_注釈)
CALL COLUMN_RIGHT_PRINTL
CALL COLOR_PRINT(" 現在の管理者は", カラー_注釈)
IF ID_TO_CHARA(FLAG:触手部屋管理者) == -1
	FONTSTYLE 1
	CALL COLOR_PRINT(" いません ", カラー_注釈)
	FONTSTYLE 0
ELSEIF IS_SYOKUSYU_MANAGE(ID_TO_CHARA(FLAG:触手部屋管理者))
	CALL COLOR_PRINT(@"%ANAME(ID_TO_CHARA(FLAG:触手部屋管理者))% ", カラー_オレンジ, "です", カラー_注釈)

ELSE
	CALL COLOR_PRINT(@"%ANAME(ID_TO_CHARA(FLAG:触手部屋管理者))% ", カラー_オレンジ, "ですが、現在は", カラー_注釈, "管理不能", カラー_選択中, "です", カラー_注釈)
ENDIF
CALL COLUMN_RIGHT_PRINTL
CALL COLUMN_RIGHT_LINE
CALL COLUMN_RIGHT_PRINTL
...
IF ARG:0 >= 0 && IS_SYOKUSYU_MANAGE(ARG:0)
	IF ARG:0 == ID_TO_CHARA(FLAG:触手部屋管理者)
		PRINTFORMW %ANAME(ARG:0)%を触手部屋の管理者から外しました
		FLAG:触手部屋管理者 = 0
	ELSE
		PRINTFORMW %ANAME(ARG:0)%を触手部屋の管理者に設定しました
