# SHOP/SHOP_LIFE/領地探索/領地探索_通常/EXPENSIVE_SHOPPING_高い買い物.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_LIFE/領地探索/領地探索_通常/EXPENSIVE_SHOPPING_高い買い物.ERB`

类型: .ERB

自动摘要: functions: TERRITORY_EVENT_EXPENSIVE_SHOPPING_EXISTS, TERRITORY_EVENT_EXPENSIVE_SHOPPING_DECISION, TERRITORY_EVENT_EXPENSIVE_SHOPPING; UI/print
...
SELECTCASE RESULT
	...
	CASE 2
		...
		ELSE
			...
			CALL FUCK(対象, "欲望, 性技, 奉仕, 口淫, 精愛", "キス喪失, 口内射精, CFLAG減少", 0, "店主のペニス", "", "", "売春")
			PRINTFORML 
	...
	CASE 3
		...
		IF (HAS_PENIS(対象) && RAND:2) || IS_MALE(対象)
			...
			CALL FUCK(対象, "欲望, 性交, 奉仕, 性技, Ｃ, 射精, 売春" + (IS_FEMALE(対象) ? ", レズ, Ｂ, Ｖ" # ""), "キス喪失, 童貞喪失", 0, @"店主の\@ RAND:2 ? 唇 # 秘貝\@", "", "店主の膣", "売春")
		ELSEIF IS_FEMALE(対象)
			PRINTFORMW 女店主は驚いた様子だったが、%ANAME(対象)%の身体を眺めると頬を赤らめ、店の奥へ案内した
			PRINTFORML 
			...
			CALL FUCK(対象, "欲望, 性交, 奉仕, 性技, Ｂ, Ｖ, Ｍ, Ｃ, レズ, 売春", "キス喪失", 0, @"店主の\@ RAND:2 ? 唇 # 秘貝\@", "", "", "売春")
		ELSE
			...
			CALL FUCK(対象, "欲望, 性交, 奉仕, 性技, Ｂ, Ｖ, Ｍ, Ｃ, Ａ, Ｖ性交, Ａ性交, 売春", "キス喪失, 処女喪失, Ａ処女喪失, 膣内射精, 腸内射精, CFLAG減少", GET_SPERM_ID("店主"), @"店主の\@ RAND:2 ? 唇 # ペニス\@", "店主", "", "売春")
		ENDIF
		PRINTFORML 
