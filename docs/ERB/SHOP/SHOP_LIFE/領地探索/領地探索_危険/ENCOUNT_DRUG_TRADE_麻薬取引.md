# SHOP/SHOP_LIFE/領地探索/領地探索_危険/ENCOUNT_DRUG_TRADE_麻薬取引.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_LIFE/領地探索/領地探索_危険/ENCOUNT_DRUG_TRADE_麻薬取引.ERB`

类型: .ERB

自动摘要: functions: TERRITORY_EVENT_ENCOUNT_DRUG_TRADE_EXISTS, TERRITORY_EVENT_ENCOUNT_DRUG_TRADE_DECISION, TERRITORY_EVENT_ENCOUNT_DRUG_TRADE; UI/print
...
IF RESULT == 0
	...
	ITEM:麻薬 += 1
	CALL COLOR_PRINTW("麻薬を1個手に入れた", カラー_注意)
	RETURN 1
ELSEIF RESULT == 2
...
IF RESULT == 0 
	IF IS_MALE(対象)
		...
		ITEM:麻薬 += 2
		CALL COLOR_PRINTW("麻薬を2個手に入れた", カラー_注意)
		RETURN 1
	ELSE
		PRINTFORML こんなことを許していては治安が乱れてしまう
...
CALL FUCK(対象, "欲望, 奉仕, 精愛, 性技, 性交, Ｃ, Ｖ, Ｂ, Ａ, Ｍ, キス, 口淫, 薬物依存, Ｖ性交, Ａ性交", "処女喪失, Ａ処女喪失, 膣内射精, キス喪失, 腸内射精, 口内射精, CFLAG減少", GET_SPERM_ID("売人"), @"売人の\@RAND:2 ? ペニス # 唇\@", "売人", "ねぐらでの輪姦")
PRINTFORML 
