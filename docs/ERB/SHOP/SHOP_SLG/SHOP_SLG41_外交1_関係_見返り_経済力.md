# SHOP/SHOP_SLG/SHOP_SLG41_外交1_関係_見返り_経済力.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_SLG/SHOP_SLG41_外交1_関係_見返り_経済力.ERB`

类型: .ERB

自动摘要: functions: DIPLOMACTY_REQUESTED_ECONOMY; UI/print

前 200 行源码片段:

```text
﻿;-------------------------------------------------
;経済力の要求の処理
;ARG:0は相手の勢力番号、ARG:1は要求の重さ。要求を飲むと1、断ると0を返す
;-------------------------------------------------
@DIPLOMACTY_REQUESTED_ECONOMY(ARG:0, ARG:1)
;相手勢力の君主のキャラ番号を取得
LOCAL:5 = GET_COUNTRY_BOSS(ARG:0)

LOCAL:12 = GET_SUM_ECONOMY(CFLAG:MASTER:所属)
IF ARG:1 == 0
	LOCAL:11 = LOCAL:12 / 80 / 100 * 100
ELSEIF ARG:1 == 1
	LOCAL:11 = LOCAL:12 / 20 / 100 * 100
ELSE
	LOCAL:11 = LOCAL:12 / 7 / 100 * 100
ENDIF
PRINTFORML %ANAME(LOCAL:5)%は、こちらの提案を飲む代わりに経済力{LOCAL:11 / 100}を要求してきた
PRINTFORML 現在の経済力:{LOCAL:12 / 100}
WAIT
CALL ASK_YN("受け入れる", "断る")
IF RESULT != 0
	RETURN 0
ENDIF

;経済力の移動処理
CALL SHIFT_ECONOMY(CFLAG:MASTER:所属, ARG:0, LOCAL:11)

RETURN 1

```
