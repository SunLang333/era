# SHOP/SHOP_SLG/SHOP_SLG41_外交1_関係_見返り_士官.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_SLG/SHOP_SLG41_外交1_関係_見返り_士官.ERB`

类型: .ERB

自动摘要: functions: DIPLOMACTY_REQUESTED_CHARA, DIPLOMACY_REQUESTED_CHARA_SELECT; UI/print
...
IF ARG:1 == 0 || ARG:1 == 1
	PRINTFORMW %ANAME(LOCAL:5)%は、こちらの提案を飲む代わりに有能な士官を一人引き渡せと要求してきた
	IF ARG:1 == 0
...
ELSE
	...
	ELSE
		PRINTFORMW %ANAME(LOCAL:5)%は、こちらの提案を飲む代わりに%ANAME(LOCAL:10)%を引き渡せと要求してきた
		CALL ASK_YN("受け入れる", "断る")
		IF RESULT == 0
			PRINTFORML %ANAME(LOCAL:10)%は%ANAME(LOCAL:5)%勢力の士官になりました…
