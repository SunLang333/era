# SHOP/SHOP_SLG/SHOP_SLG71_出頭.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP_SLG/SHOP_SLG71_出頭.ERB`

类型: .ERB

自动摘要: functions: SHOP_SLG_NAME71, SHOP_SLG_CHECK71, SHOP_SLG_EVENTBUY71, SHOP_SELF_SURRENDER, SELECT_CHARA_LIST_SELECT_LOGIC_SHOP_SELF_SURRENDER, SELECT_CHARA_RANDOM_LOGIC_SHOP_SELF_SURRENDER, SELECT_CHARA_RANDOM_BIAS_SHOP_SELF_SURRENDER; assigns RESULTS; UI/print
...
CALL SINGLE_DRAWLINE
PRINTFORML %ANAME(GET_COUNTRY_BOSS(勢力))%勢力に出頭し、降伏し捕虜になります
PRINTFORML 誰が出頭しますか？（未選択のまま『これでよし』でキャンセル）
CALL SINGLE_DRAWLINE
...
IF FINDELEMENT(SELECTED_CHARA, MASTER, 0, SELECTED_CHARA_NUM) != -1
	...
	PRINTFORMW ・
	PRINTFORMW ・
	PRINTFORML 出頭した%ANAME(MASTER)%を出迎えたのは%ANAME(LOCAL:1)%だった
	PRINTFORML %ANAME(LOCAL:1)%は驚いた目で%ANAME(MASTER)%を迎えつつ、
	PRINTFORMW 捕虜としての%ANAME(MASTER)%の世話は自分がこちらで引き受けると宣言した……
...
FOR LOCAL, 0, SELECTED_CHARA_NUM
	...
	PRINTFORML %ANAME(LOCAL:1)%は%ANAME(GET_COUNTRY_BOSS(勢力))%に出頭した……
