# SHOP/SHOP.ERB — 自动生成文档

源文件: `ERB/SHOP/SHOP.ERB`

类型: .ERB

自动摘要: functions: EVENTSHOP, SHOW_SHOP, USERSHOP, EVENTBUY; UI/print
...
IF TIME == 0
	...
	IF COOLTIME:MASTER:0 >= 2
		CALL COLOR_PRINT(@"ｸｰﾙﾀｲﾑ{COOLTIME:MASTER:0}", カラー_赤)
...
ELSE
	IF IS_COUNTRY(CFLAG:MASTER:所属)
		...
		RESETCOLOR
		PRINTFORM /\@ STRLENS(TOSTR(GET_RECRUIT_LIMIT(CFLAG:MASTER:所属))) > 6 ? たくさん # %NUM_FORMAT(GET_RECRUIT_LIMIT(CFLAG:MASTER:所属))% \@
