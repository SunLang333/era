# SKILLS/SKILL_SWAP.ERB — 自动生成文档

源文件: `ERB/SKILLS/SKILL_SWAP.ERB`
...
PRINTFORML ◆%ANAME(対象)%のスキル◆
PRINTFORML ※同一ジャンル間でのみ入れ替えられます
LOCAL:2 = 0
FOR LOCAL, 0, SKILL_GENRE_NUM
	FOR LOCAL:1, 0, MAX_SKILL_SLOT
		...
		ELSE
			RESETCOLOR
			PRINTBUTTON "[入れ替え]", LOCAL * 1000 + LOCAL:1
		ENDIF
		RESETCOLOR
